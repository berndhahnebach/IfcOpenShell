...
# This can be packaged with `pyinstaller --onefile --hidden-import numpy --collect-all ifcopenshell --clean brep2ifc.py`
import argparse
import ifcopenshell
...
class Brep2Ifc:
    ...
    def _execute(self):
        abrep_file = open(self.paths, "r")
        brep_geom = abrep_file.read()
        abrep_file.close()
        representation = geom.serialise("IFC4", brep_geom)
...
if __name__ == "__main__":
    ...
    brep2ifc = Brep2Ifc(args.obj, args.output)
    brep2ifc.execute()
