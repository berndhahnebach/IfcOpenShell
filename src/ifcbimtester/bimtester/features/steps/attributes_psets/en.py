...
@step('All "{ifc_class}" elements have exactly "{attribut_count}" in the pset "{pset}"')
...
@step('All "{ifc_class}" elements have a "{pset}.{aproperty}" property')
def step_impl(context, ifc_class, pset, aproperty):
...
@step('All "{ifc_class}" elements have not a "{pset}.{aproperty}" property')
def step_impl(context, ifc_class, pset, aproperty):
...
@step('All "{ifc_classes}" elements have a property "{aproperty}" in the Common pset')
...
@step('All "{ifc_class}" elements with a "{pset}.{aproperty}" are of type "{propertytyp}"')
...
@step('All "{ifc_class}" elements with a "{pset}.{aproperty}" have a value of "{propertyvalue}"')
...
@step('All "{ifc_class}" elements with a "{pset}.{aproperty}" have a value range of "{valuerange}"')
...
@step('All "{ifc_class}" elements with a "{pset}.{aproperty}" have the chars "{some_chars}" not in the property value"')
...
@step('All "{ifc_class}" elements have no element layer properties (IfcComplexProperty)')
...
@step('All "{ifc_class}" elements with the material named "{material}" have a "{pset}.{property}" property')
...
@step('All "{ifc_class}" elements which do not have a material named "{material}" have not a "{pset}.{property}" property')
def step_impl(context, ifc_class, material, pset, property):
...
def eleclass_minus_has_property_in_pset(
    context, ifc_class, target_pset, target_property, minus_ifc_class=""
):
    ...
    # get the elements
    all_elements = IfcStore.file.by_type(ifc_class)
    elements = []
    if minus_ifc_class == "":
...
def eleclass_has_property_in_common_pset(
    context, ifc_classes_string, target_property
):
...
def eleclass_has_propertytype_of(
    context, ifc_class, target_pset, target_property, target_propertytype
):
    ...
    for elem in elements:
        ...
        for propvaltyp in get_prop_values_and_types(elem, target_pset, target_property):
            ...
            if actual_propertytype != target_propertytype:
                ...
                context.falseelems.append("{}, {}.{} = {}, {}:".format(elem, target_pset, target_property, actual_value, actual_propertytype))
...
def eleclass_has_propertyvalue_of(
    context, ifc_class, target_pset, target_property, target_value
):
        ...
        for propvaltyp in get_prop_values_and_types(elem, target_pset, target_property):
            ...
            if actual_value != target_py_value:
                ...
                context.falseelems.append(
                    "{}, {}.{} = {} ({}), targettype eval = {}:"
                    .format(elem, target_pset, target_property, actual_value, actual_propertytype, type(target_py_value))
                )
        if elem_has_false_prop is True:
            context.falseguids.append(elem.GlobalId)
            context.falseprops[elem.id()] = str(IfcStore.psets[elem.id()])
...
def eleclass_has_property_valuerange_of(
    context, ifc_class, target_pset, target_property, target_valuerange
):
    ...
    for elem in elements:
        ...
        for propvaltyp in get_prop_values_and_types(elem, target_pset, target_property):
            ...
            if actual_value not in target_py_valuerange:
                ...
                context.falseelems.append(
                    "{}, {}.{} = {} ({}), targetrange eval = {}:"
                    .format(elem, target_pset, target_property, actual_value, actual_propertytype, target_py_valuerange)
                )
        if elem_has_false_prop is True:
            context.falseguids.append(elem.GlobalId)
            context.falseprops[elem.id()] = str(IfcStore.psets[elem.id()])
...
def eleclass_hasnot_chars_in_property_value(
    context, ifc_class, target_pset, target_property, target_chars
):
        ...
        if props[0] is True:
            for prop in props[1]:
                ...
                if target_chars in actual_value:
                    ...
                    context.falseelems.append("{}, {}.{} = {}:".format(elem, target_pset, target_property, actual_value))
                    context.falseguids.append(elem.GlobalId)
                    context.falseprops[elem.id()] = str(IfcStore.psets[elem.id()])
...
def eleclass_has_propertycount_in_pset(
    context, ifc_class, target_attribut_count, target_pset
):
...
def eleclass_has_property_in_pset(
    context, ifc_class, pset, aproperty
):
    context.falseelems = []
    context.falseguids = []
    context.falseprops = {}
