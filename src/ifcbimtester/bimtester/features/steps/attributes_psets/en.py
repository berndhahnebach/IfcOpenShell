...
@step('All "{ifc_class}" elements with a "{aproperty}.{pset}" are of type "{propertytyp}"')
...
@step('All "{ifc_class}" elements with a "{aproperty}.{pset}" have a value of "{propertyvalue}"')
...
@step('All "{ifc_class}" elements with a "{aproperty}.{pset}" have a value range of "{valuerange}"')
...
@step('All "{ifc_class}" elements with a "{aproperty}.{pset}" have the chars "{some_chars}" not in the property value"')
...
def eleclass_has_no_complex_property(
    ...
    for elem in elements:
        if has_elem_complex_props(elem) is True:
            ...
            context.falseprops[elem.id()] = str(eleutils.get_psets(elem, psets_only=True))
...
def eleclass_minus_has_property_in_pset(
    ...
    for elem in elements:
        psets = eleutils.get_psets(elem, psets_only=True)
    ...
    for elem in elements:
        ...
        if (
            ...
            if not prop_value1 == prop_value2:
                ...
                falseprops[elem.id()] = str(eleutils.get_psets(elem, psets_only=True))
