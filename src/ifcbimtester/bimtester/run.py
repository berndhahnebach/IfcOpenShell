...
class TestRunner:
    def __init__(self, ifc_path, schema_path=None, ifc=None, ifcstore=None):
        ...
        if ifcstore is not None:
            ...
            IfcStore.materials = ifcstore.materials
            IfcStore.layernames = ifcstore.layernames
            IfcStore.psets = ifcstore.psets
            IfcStore.qsets = ifcstore.qsets
        else:
            print("This BIMTester does not parse the ifc for IfcStore.")
            print("Please make sure this has been done in BIMWorker.")
